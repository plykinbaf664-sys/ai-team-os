...
const PLANNER_INSTRUCTIONS = [
  ...
  "Запросы «сверь», «синхронизируй», «приведи в порядок», «подчисти», «поправь сам» являются запросами на реальное выполнение, даже если одновременно требуют анализа стратегии. На первом проходе прочитай и TickTick через list_tasks, и релевантные таблицы через read_sheet; после чтения верни ready с конкретными write actions, а не response с рекомендациями.",
  "При операционной сверке каждое изменение TickTick привязывай к точному taskId из результатов list_tasks. Каждое изменение таблицы привязывай к spreadsheetId и существующей строке из результатов read_sheet. Не выбирай первую задачу из списка и не создавай техническую заметку вместо реального изменения статуса, приоритета, срока или завершения.",
  "Не пиши в contentNote, что задача удалена, завершена, очищена или системы синхронизированы, если соответствующее действие фактически не выполняется. Если задача подтверждённо больше не актуальна — используй complete_task; если она поставлена на паузу — обнови только подтверждённые поля и добавь фактическую причину в заметку.",
  "Небольшие однозначные корректировки статусов, приоритетов, дат, комментариев и связанных строк выполняй автоматически. Удаление, очистка, структура, формулы и массовые изменения оставляй за confirmation policy.",
  ...
  "Когда нужно найти существующую задачу перед изменением, используй list_tasks с query по имени или устойчивой части названия и limit=50. После чтения выбирай точный taskId по названию, содержимому карточки и контексту разговора; при равных совпадениях задай вопрос.",
  ...
  "changes.project у update_task означает реальный перенос карточки. Заполняй его только когда пользователь явно попросил перенести задачу в другой проект или список; не выводи проект из темы разговора.",
  "Если прочитанная карточка уже содержит сообщённый факт, не добавляй его повторно. Верни готовый результат без write action: данные уже актуальны.",
...
> = {
  ...
  list_tasks: ["project", "query", "limit"],
  create_calendar_event: [
    "title",
    "date",
    "startTime",
...
export function isStrategicAnalysisRequest(sourceText: string) {
  ...
  return requestsStrategicOutput(sourceText) && referencesBusinessData && !asksToChangeData;
}

export function requestsStrategicOutput(sourceText: string) {
  return /проанализ|аналитик|стратег|слаб\w*\s+мест|узк(?:ое|ие|их)?\s+(?:мест|горлыш)|бутылоч|точк[аи]\s+рост|приоритет|пошагов|план\s+действ|быстр\w*\s+ден|получить\s+ден|заработ|доход|прибыл|что\s+делать|куда\s+двиг|выдай\w*\s+(?:мне\s+)?(?:решен|стратег|план)/iu.test(
    sourceText.toLocaleLowerCase("ru"),
  );
}
